			    ...
			    struct mtd_oob_ops *ops)
{
	...
	nanddev_for_each_page(nand, from, ops, &iter) {
		...
		if (iter.req.ooblen) {
			memcpy(ops->oobbuf + ops->oobretlen,
			       spinand->oobbuf + iter.req.ooboffs,
			       iter.req.ooblen);
			     ...
			     struct mtd_oob_ops *ops)
{
	...
	mutex_lock(&spinand->lock);
	nanddev_for_each_page(nand, to, ops, &iter) {
		memset(spinand->buf, 0xff,
		       nanddev_page_size(nand) +
		       nanddev_per_page_oobsize(nand));

		if (ops->datbuf)
			memcpy(spinand->buf + iter.req.dataoffs,
			       ops->datbuf + ops->retlen, iter.req.datalen);

		if (ops->oobbuf)
			memcpy(spinand->oobbuf + iter.req.ooboffs,
			       ops->oobbuf + ops->oobretlen, iter.req.ooblen);
